...
#MOst Words they used  WOrldCloud
#THe Emotion of their speech
#length of their speech
```
...
sentence.list = list(L1,L2,W1,W2,F2,F3,F4,J1,R1)
title.list = c(rep("AbrahamLincoln-1",length(L1)),rep("AbrahamLincoln-2",length(L2)),rep("WoodrowWilson-1",length(W1)),rep("WoodrowWilson-1",length(W2)),rep("FranklinDroosevelt-2",length(F2)),rep("FranklinDroosevelt-3",length(F3)),rep("FranklinDroosevelt-4",length(F4)),rep("JohnFKenndy-1",length(J1)),rep("RonaldRegan-2",length(R1)))
emotion.matrix = cbind(title.list,c(L1,L2,W1,W2,F2,F3,F4,J1,R1))
...
for(i in 1 : length(sentence.list))
{
 ...
 interm.matrix = rbind(interm.matrix,cbind(sent.id=as.numeric(1:length(sentence.list[[i]])),nword=as.numeric(word_count(sentence.list[[i]])),as.matrix(emotions)))
}
emotion.matrix = as.data.frame(cbind(emotion.matrix,interm.matrix))
```

```{r}
return_short_sentence=function(df=emotion.matrix,Presidents,nwords=5)
{
  return(df$Sentences[which(df$President==Presidents && as.numeric(df$nword) <= nwords)])
}
return_short_sentence(President = "AbrahamLincoln-1")
return_short_sentence(President = "AbrahamLincoln-2")
#WoodrowWilson-1
return_short_sentence(President = "WoodrnowWilson-1")
...
setwd("D:/Columbia University/Spring2017-Applied Data Science/Project_1_Bz2290/Spr2017-Proj1-bz2290/data")
...
#JohnFKenndy-1 35
#RonaldReagan-1 41
#inaugRichardNixon-1 39
#inaugRichardNixon-1 40  VIetnam War
#GeorgeBush-2 21 Iraq War
