---
title: "A comparision between war time speech and peacful time speech"
output:
...
There is no doubt that the top three emotions in each of these four speeches are the same, and the rank of other emotions are not very different.Something else worth noticing is that "fear" always comes in fourth. Having postive emotions in the top is not out of our expectations, but we should dig a little bit deeper of the sentences that express "fear" of the speakers' attitudes. We also observed another interesting pattern when we examined a partial list of barplot from the "war" group:
```{r, echo=FALSE}
par(mfrow=c(2,2))
...
Surprisingly, positive emotions still makes up the top two emotions in these speeches, and there are only one speeches whose top emotion is not positive, which is the second speech given by Lincoln, towards to the end of the civil war:
```{r, echo=FALSE}
par(mfrow=c(1,1))
Emotion_Barplot(speech.selected.war[2])
```

Compared to Lincoln's first speech, we see that the rank of negative emotions actually goes up even the civil war is about to end, but another president who has a similiar experience, of course, this time, the second world war, has a differnt change of emotions during his speech:
```{r, echo=FALSE}
par(mfrow=c(2,2))
...
For now, we have observed that emotion "fear" always comes fourth in our barplot of emotions for the "peace" group, the emotion of presidential speeches during war time is surprisingly positive, and two presidents whose speeches were given thorought out wars have opposite change on emotion "fear".In order to shed lights into these patterns, I decided to utlize some more techniques to derive more insights.

I decided to examine the sentences that carry the emotions of interest to see if I can find more interesting things.
First, let us examine the "fear" sentences from presiednts in the "peace" group:
```{r, echo=FALSE}
...
The second cluster plot seems to be better and we see that general emotion from FranklinDRoosevelt speeches changes dramatically, as his three speeches belong to three different clusters seperately.
